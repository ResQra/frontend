...
const EVENT_ICONS = {
  incident_received: { icon: AlertTriangle, color: 'text-red-400' },
  priority_scored: { icon: Radio, color: 'text-amber-400' },
  status_changed: { icon: CheckCircle2, color: 'text-sky-400' },
  team_assigned: { icon: Truck, color: 'text-violet-400' },
  allocation_recommended: { icon: Users, color: 'text-cyan-400' },
  pending_action_approved: { icon: CheckCircle2, color: 'text-emerald-400' },
  pending_action_rejected: { icon: AlertTriangle, color: 'text-red-400' },
  sensor_event_received: { icon: Radio, color: 'text-pink-400' },
  location_identified: { icon: MapPin, color: 'text-green-400' },
  location_unverified: { icon: MapPin, color: 'text-amber-400' },
  risk_density_recomputed: { icon: AlertTriangle, color: 'text-orange-400' },
  team_location_updated: { icon: Truck, color: 'text-blue-400' },
}
  ...
  return (
    <div className="flex gap-3">
      <div className="flex flex-col items-center">
        <div className={`grid size-7 place-items-center rounded-full bg-slate-800 ${meta.color}`}>
